...
function smtp_mail($to, $subject, $message, $from_name, $from, $cc, $bcc, $debug=false) {
  ...
  $mail->SMTPAuth 	= true;                        				
  $mail->Host 		  = 'fikti.bem.gunadarma.ac.id;118.98.72.14';  // Masukkan Server SMTP
  $mail->Port 		  = 587;                                      // Masukkan Port SMTP
  $mail->SMTPSecure = 'tls';                                    // Masukkan Pilihan Enkripsi ( `tls` atau `ssl` )
  $mail->Username 	= 'farhanalfayyadh@fikti.bem.gunadarma.ac.id';                // Masukkan Email yang digunakan selama proses pengiriman email via SMTP
  $mail->Password 	= 'Indonesia2000';        						          // Masukkan Password dari Email tsb
  $default_email_from       = 'farhanalfayyadh@fikti.bem.gunadarma.ac.id';        // Masukkan default from pada email
  $default_email_from_name  = 'BEM FIKTI (Biro PTI)';           // Masukkan default nama dari from pada email
